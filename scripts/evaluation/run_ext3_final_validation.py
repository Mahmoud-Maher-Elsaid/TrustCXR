...
def main() -> int:
    ...
    if (
        config["lock_policy"]["final_test_evaluation_authorized"]
        or config["lock_policy"]["locked_test_accessed"]
    ):
        raise RuntimeError("EXT-3 locked-test protection is disabled.")
    manifest_path = root / config["cohort"]["manifest_path"]
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if (
        manifest.get("manifest_sha256") != manifest_hash(manifest)
    ...
    checkpoint = (
        (root / args.checkpoint).resolve() if not args.checkpoint.is_absolute() else args.checkpoint
    )
    ...
    summary = {
        "stage": "EXT-3 FINAL",
        "decision": "PASS_DEVELOPMENT_GATE" if selected else "FAIL_OPERATING_POINT",
        ...
        "detected_lesion_count": sum(
            len(item["true_positives"]) + len(item["false_positives"]) for item in selected_matches
        ),
        "metrics": ap,
        "threshold_grid": list(THRESHOLDS),
        "operating_point_status": operating_status,
        "selected_operating_point": selected,
        "threshold_table": threshold_tables,
        "bootstrap_ci": patient_bootstrap(
            selected_matches,
            analysis_threshold,
            config["metrics"]["bootstrap"]["seed"],
            config["metrics"]["bootstrap"]["replicates"],
        ),
    ...
    (output / "threshold_table.json").write_text(
        json.dumps(threshold_tables, indent=2) + "\n", encoding="utf-8"
    )
