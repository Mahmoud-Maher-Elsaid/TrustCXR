...
Push-Location $repo
try {
& $python -m pytest -q tests/unit/test_ext4e2d_candidate1_dev_smoke.py tests/unit/test_ext4e2d0_structured_output_compatibility.py
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $python -m pytest -q
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $python (Join-Path $repo "scripts\validation\validate_ext4e2d_pre_inference.py")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
} finally {
 Pop-Location
}
Write-Output "EXT-4E2D PRE-INFERENCE GATE: PASS"
