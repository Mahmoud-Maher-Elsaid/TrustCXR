...
\subsection{Frozen core and post-freeze extensions}
The frozen core comprises the vision models, governed evidence packaging,
predictive-uncertainty contracts, deterministic Stage 18--20 reporting and
verification, and the expert-review interface. Post-freeze EXT-4 experiments
were a separate, research-only extension and did not change frozen-core model
selection or access final/locked records. EXT-4E evaluated multiple LLM
candidates and selected none. EXT-4F strengthened semantic contracts and
constrained realization, but its candidate was not selected. EXT-4G Gemma 3
passed technical runtime preparation but failed the development selection
gate. EXT-4H replaced whole-envelope generation with deterministic slot
orchestration: its automatic structural/technical gate passed, while the
blinded EXT-4H.5 semantic review failed (513/765 applicable decisions passed,
21/59 reviewed slots passed, 0/24 cases passed, and the optimistic semantic
ceiling was 701/765 = 0.9163). Thus unresolved review judgments could not
rescue selection. EXT-4I identified the resolved failures as LLM surface-
realization drift and implemented semantic atoms, proposition boundaries,
phrase skeletons, and a deterministic validator; its 16-case historical
regression and 264-mutation safety suite passed. A bounded lexical LLM role
was then considered but provided no demonstrated scientific benefit over
deterministic lexical selection. The final project therefore retains fully
deterministic semantically bounded realization.
...
\label{tab:scope}
\begin{tabularx}{\columnwidth}{@{}>{\raggedright\arraybackslash}p{0.28\columnwidth} >{\raggedright\arraybackslash}p{0.18\columnwidth} Y@{}}
\toprule
...
The TrustCXR project is closed and no next stage is authorized. Historical
post-release governance remains documented for provenance, but it does not
authorize implementation or imply a current capability. Any future,
independently governed study would need a new project decision. If visual
attribution were ever reconsidered, Grad-CAM maps would be described only as
model attention or visual attribution, never as validated lesion location
\citep{selvaraju2017gradcam}; true pathology localization would require
independent spatial annotations. The grounded-LLM path was explicitly tested
and not retained for final realization.

\section{Conclusion}
TrustCXR demonstrates a conservative way to package chest X-ray research models for expert-oriented review. Its contribution is the combination of frozen perception and classification, predictive-uncertainty evidence, provenance-aware evidence handling, deterministic reporting and verification, and a defer-first decision policy within a reproducible local application. The evidence supports a research system with explicit boundaries, not a clinical diagnostic product. The post-freeze LLM experiments are negative evidence rather than a selected capability: structural orchestration alone did not establish semantic faithfulness, and the final deterministic semantic-boundary architecture was retained after regression validation. External validation, reliable positive localization, active learning, and treatment or diagnostic use remain outside scope. The project is closed; no next stage is authorized.
...
Extension roadmap & Historical post-release governance and results; no next stage authorized\\
\bottomrule
\end{tabularx}
\end{center}
\vspace{3pt}
Reconstruction is anchored by the final Windows/CUDA environment lock and its installation protocol. The local service accepts bounded raster input, validates decoding, runs the view/quality and classifier models sequentially on the governed GPU, and releases model residency before deterministic downstream processing. This ordering is an operational reproducibility constraint, not a claim of production deployment.

The evidence boundary is equally important: raw datasets remain outside Git, patient-safe split records and locked-test protections are preserved, and the release does not include external validation, human-feedback acquisition, attribution maps, pathology localization, or language-model reporting. The final deterministic realization decision and its negative LLM history are preserved in the EXT-4 reports. These boundaries are part of the reproducibility result because they define what another authorized researcher is, and is not, being asked to reconstruct.
\end{multicols}
