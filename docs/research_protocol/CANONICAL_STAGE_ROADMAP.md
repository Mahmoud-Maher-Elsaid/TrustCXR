...
| 11C | Pending | Hold for official identity-mapping audit | Conservative label adjudication and local official-mapping discovery | Shared-cohort resolution and label harmonization | Original Stage 8 | Ready to run | Reject label equivalence, document partial-support semantics, and identify required official mapping | 11D |

## Numbering rule

Stage 11C is the next valid identifier. It reads aggregate evidence and checks only for a local official mapping artifact; it performs no training, image inference, or locked-test access. The semantic relation is limited to possible-pneumonia opacity partially supporting, but never confirming or equating to, NIH `Pneumonia`. Record-level fusion remains prohibited until the official RSNA-to-NIH mapping is acquired and separately audited.
