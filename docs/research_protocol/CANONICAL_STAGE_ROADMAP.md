...
| 10M | Pending | Determined by validation geometry evidence | Validation-only detection geometry and thoracic-location proxy audit | Localization anatomical-validity proxy | Original Stage 7 | Ready to run | Aggregate image-bound, edge-margin, and spatial-distribution audit using the selected baseline | 10N or repair hold |

## Numbering rule

Stage 10M is the next valid identifier. It performs validation-only inference with the selected Stage 10E checkpoint and reports aggregate box geometry, image-bound violations, edge-margin contact, and a coarse spatial distribution. Because matched RSNA anatomy masks are unavailable, this is explicitly an anatomical-location proxy rather than validated lung-region localization. It performs no training and keeps the final split locked.
