...
| 11A | Pending | Go for Stage 11B fusion data-contract validation | No-inference structured evidence-fusion contract | Evidence status and disagreement semantics | Original Stage 8 | Ready to run | Freeze label mapping, localization scope, test locks, and shared-cohort requirement | 11B |

## Numbering rule

Stage 11A is the next valid identifier. It performs no training or inference. It freezes structured evidence statuses and disagreement semantics while preserving the Stage 10 rule that absent localization cannot contradict a positive classifier. Only Pneumonia has a Stage 10 localization contract, and record-level cross-dataset fusion remains prohibited until a shared cohort is proven.
