...
The protocol `1.0.0` input-rejection review contains exactly 12 class/split slots. Two validation slots are approved from objective adjudication evidence: `INCOMPLETE_ANATOMY` and `INADEQUATE_QUALITY`. The remaining ten slots are explicitly recorded as `INCOMPLETE_NO_DEFENSIBLE_EXAMPLE`.
